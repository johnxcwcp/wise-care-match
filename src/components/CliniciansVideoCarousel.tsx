...
const CliniciansVideoCarousel: React.FC<CliniciansVideoCarouselProps> = ({ therapists }) => {
  const [playingVideo, setPlayingVideo] = useState<{ videoId: string; therapistName: string } | null>(null);
  ...
  const handlePlayVideo = (url: string, name: string) => {
    ...
    if (videoId) {
      setPlayingVideo({ videoId, therapistName: name });
    }
  };
                  ...
                  <div className="bg-white rounded-xl shadow-md overflow-hidden border border-cwcp-gray hover:shadow-lg transition-shadow duration-300">
                    <div className="p-6">
                      <div className="flex flex-col">
                        ...
                        <div className="w-full aspect-video rounded-lg overflow-hidden mb-4 relative group">
                          {videoId ? (
                            <>
                              <img
                              ...
                              <div className="absolute inset-0 bg-black bg-opacity-40 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                                <Button
                                  ...
                                  className="bg-white bg-opacity-90 hover:bg-opacity-100 text-cwcp-blue rounded-full p-4"
                                >
                                  <Play size={24} />
                                </Button>
                              </div>
